...
def get_mask_from_contour(shape, contour_list):
    ...
    for polygon in contour_list:
        ...
        if pts.ndim == 1: continue # degenerate
...
def main():
    ...
    image_files = ['/home/bl515-01/sam3/b.jpg', '/home/bl515-01/sam3/c.jpg', '/home/bl515-01/sam3/d.jpg', '/home/bl515-01/sam3/a.jpg'] 
    # Let's put 'a.jpg' as the reference view (last in list -> index 3)
    contours_file = '/home/bl515-01/sam3/a_contours.json'
    ...
    # Reference view data (a.jpg at index 3)
    pts3d_ref = pts3d_all_views[3]
    H_model, W_model = pts3d_ref.shape[:2]
    ref_img = Image.open(image_files[3])
    ...
    if len(card_pts) < 10:
        print("Error: Card pts insufficient for calibration.")
        return
    ...
    # 3. Multi-View Thickness Integration
    # ----------------------------------
    cam_poses = scene.get_im_poses()
    K = scene.get_focals()
    inv_pose_ref = torch.inverse(cam_poses[3]).detach().cpu().numpy()
    focal_ref = K[3].detach().cpu().item()
    ...
    for food, contour_list in contours_data.items():
        ...
        thick = np.where((max_h[valid_bins] - min_h[valid_bins]) > (0.005 / cm_per_model_unit), 
                         max_h[valid_bins] - min_h[valid_bins], 
                         max_h[valid_bins])
        
        dx = np.median(np.linalg.norm(pts3d_ref[:, 1:] - pts3d_ref[:, :-1], axis=2))
        dy = np.median(np.linalg.norm(pts3d_ref[1:, :] - pts3d_ref[:-1, :], axis=2))
        
        final_h_full = np.zeros_like(max_h)
        final_h_full[valid_bins] = thick
        vol_cm3 = np.sum(final_h_full[mask_flat]) * dx * dy * (cm_per_model_unit ** 3)
        
        results[food] = vol_cm3
        thickness_map.flat[valid_bins] = final_h_full[valid_bins]
        print(f"Food: {food:<12} Volume: {vol_cm3:.2f} cm^3")

    with open("/home/bl515-01/sam3/results_a_d.json", 'w') as f:
        json.dump(results, f, indent=2)
    
    max_vt = np.max(thickness_map)
    if max_vt > 0:
        vis = cv2.applyColorMap((thickness_map / max_vt * 255).astype(np.uint8), cv2.COLORMAP_JET)
        vis[thickness_map == 0] = 0
        cv2.imwrite("/home/bl515-01/sam3/thickness_map_a_d.png", vis)
    print("\nProcessing of a-d.jpg complete.")
