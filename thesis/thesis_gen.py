...
para("式中，{C:infer} 為單次推理成本（US$）；{T:infer} 為單次推理時間（秒）；"
     ...
     indent_chars=0)
para("本研究已於 RTX PRO 4500 與 RTX 4090 完成常駐後實測（同一混合餐盤、相同管線）。"
...
para("此外，尚需區分「一次性冷啟動成本」與「每次推理成本」：冷啟動成本主要來自首次下載與載入"
     ...
     "以 RTX PRO 4500 常駐 20.61 秒計約 US$ 0.0042；RTX 4090 常駐 15.69 秒計約 US$ 0.0026"
     "（約新台幣 0.08 元，匯率約 32）。")
para("〔結果討論〕RTX 4090 常駐後較 PRO 4500 快約 31%（15.69 vs 20.61 秒），且每小時單價較低"
     "（約 0.60 vs 0.74），故單次推理成本更低（0.0026 vs 0.0042）。PRO 4500 之優勢在於實驗期間"
     "供貨較穩、顯存 32 GB 對常駐三模型更充裕；若平台有穩定 4090 供給，則就成本與延遲而言"
     "4090 更具效益。L4／5090 等其餘規格仍待補測。")
fig_img("fig6_2.png", "圖 6-2　已實測 GPU 之推理時間與單次成本比較", width_cm=14)
todo_note("L4／5090 等其餘 GPU 待補測後更新表 6-9 與圖 6-2；"
          "PRO 4500 與 4090 常駐數據已記入 benchmark_report.json。")
