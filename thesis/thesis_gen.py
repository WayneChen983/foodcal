...
make_table(
    ...
    [
        ["NVIDIA L4", "Ada", "24 GB", "—", "約 0.40–0.79", "首次可跑／常駐不足", "冷啟動可完成；三模型常駐 OOM"],
...
para("由表 5-3 可知，本管線對 GPU 之需求集中於約 24 GB 以上之顯示記憶體，而非極高之運算等級；"
     "實測上 RTX 4090、RTX PRO 4500 與 RTX 5090 可於模型常駐模式下穩定服務，"
     "NVIDIA L4（同為約 24 GB）僅能完成單次序列推論、常駐再推論則 OOM。"
     "A100 與 RTX PRO 6000 等大顯存等級雖亦可執行，但其成本較高、就本應用而言運算能力有所過剩。"
...
para("關於系統之最低硬體限制與流量—配置對應規格，本研究依模型之記憶體佔用與單次推論之運算量"
     ...
     "為使單一使用者之請求得以順利完成，建議之最低 GPU 顯示記憶體為 24 GB，"
     "且需預留常駐餘裕（實測同容量之 L4 與 4090 表現不同）。於多使用者情境下，"
     "由於單次推論需時較長，服務吞吐量受限於 GPU 之數量與單次推論時間，可透過水平擴充"
     "（增加 GPU 執行個體並搭配請求佇列）以支撐更高之並行流量。")
make_table(
    ["並行使用人數／流量", "建議配置", "備註"],
    [
        ["單人測試／開發", "1 × 24–32GB GPU（如 RTX 4090／PRO 4500；L4 不宜常駐）", "循序處理，用畢即關閉 Pod"],
...
para("本研究已於 RTX PRO 4500、RTX 4090 與 RTX 5090 完成常駐後實測（同一混合餐盤、相同管線）；"
     "另於 NVIDIA L4（約 24 GB）實測：第一次完整管線可跑完，合計約 216.2 秒"
     "（VLM 161.74、SAM3 26.72、DUSt3R 26.35 秒），但第二次以模型常駐再推論時於重用"
     "Qwen2.5-VL 階段發生 CUDA out of memory／連線中斷，顯示 L4 不足以在「三模型常駐」"
     "模式下提供穩定之低延遲服務。"
     "表 6-9 以常駐後單次推理時間計算成本（L4 另列冷啟動對照）；PRO 4500 冷啟動 249.42 秒僅作對照。"
     "相對速度以 PRO 4500 常駐 20.61 秒為 1.0。")
make_table(
    ["GPU 型號", "顯示記憶體", "每小時費用(US$)", "單次推理時間(秒)", "單次推理成本(US$)", "相對速度"],
    [
        ["NVIDIA L4", "24 GB", "約 0.40–0.79", "216.2（冷／首次）；常駐 OOM", "0.035（冷）／—", "—"],
...
para("此外，尚需區分「一次性冷啟動成本」與「每次推理成本」：冷啟動成本主要來自首次下載與載入"
     ...
     "L4 僅能以冷／首次模式完成（約 US$ 0.035，時薪以 0.59 計），且無法進入常駐服務模式。")
para("〔結果討論〕就延遲而言，RTX 5090 最快（13.19 秒，約 1.56×），其次為 RTX 4090（15.69 秒，"
     ...
     "NVIDIA L4 雖時薪較低，實測顯示「單次序列載入」可完成（216.2 秒），但「三模型常駐」"
     "再推論會 OOM；故不適合作為低延遲線上服務之常駐節點。"
     "綜合延遲、成本與顯存，論文原型與小流量服務建議優先選能穩定常駐之規格"
     "（實測以 RTX 4090／PRO 4500／5090 為可行集；同為 24 GB 之 4090 可常駐，L4 則否）。")
fig_img("fig6_2.png", "圖 6-2　已實測 GPU 之推理時間與單次成本比較（L4 僅冷啟動可跑、常駐 OOM，未列入長條）", width_cm=14)
todo_note("L4：冷 216.2 s／常駐 OOM 已記入表 6-9；"
