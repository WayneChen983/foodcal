...
para("式中，{C:infer} 為單次推理成本（US$）；{T:infer} 為單次推理時間（秒）；"
     ...
     indent_chars=0)
para("本研究已於 RTX PRO 4500 完成實測；其餘規格之推理時間尚未實測，表 6-9 僅填入已量測列，"
     "並以常駐後 20.61 秒計算單次成本。冷啟動 249.42 秒對應單次成本約 US$ 0.051，"
     "僅作對照，不作為常態服務成本。")
make_table(
    ["GPU 型號", "顯示記憶體", "每小時費用(US$)", "單次推理時間(秒)", "單次推理成本(US$)", "相對速度"],
    [
        ["NVIDIA L4", "24 GB", "約 0.40–0.79", "未實測", "—", "—"],
        ["RTX 4090", "24 GB", "約 0.60", "未實測", "—", "—"],
        ["RTX PRO 4500", "32 GB", "約 0.74", "20.61（常駐）", "0.0042", "1.0（本研究）"],
        ["RTX 5090", "32 GB", "約 0.99", "未實測", "—", "—"],
...
para("此外，尚需區分「一次性冷啟動成本」與「每次推理成本」：冷啟動成本主要來自首次下載與載入"
     ...
     "以 RTX PRO 4500、常駐後 20.61 秒計，單次約 US$ 0.0042（約新台幣 0.13 元，匯率約 32）。")
para("〔結果討論〕在已實測之 RTX PRO 4500 上，常駐後單次成本極低（約 0.4 美分），回應時間約"
     "21 秒，適合作為論文原型與小流量服務之部署規格。其餘 GPU 之相對速度與成本需待補測後，"
     "始能繪製完整成本—效能比較圖；目前圖 6-2 暫以 RTX PRO 4500 單點標示。")
figure_placeholder("圖 6-2　不同 GPU 規格之推理時間與單次成本比較圖",
                   "目前僅 RTX PRO 4500 有實測點（常駐 20.61 秒、約 US$ 0.0042）；其餘規格待補測")
todo_note("其餘 GPU（L4／4090／5090 等）之單次推理時間待補測後更新表 6-9 與圖 6-2；"
          "RTX PRO 4500 冷／熱啟動數據已記入表 6-7、6-8 與 benchmark_report.json。")
